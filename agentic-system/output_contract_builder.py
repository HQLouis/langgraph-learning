...
# Import the Pydantic output-contract models.
# The models live in backend/models/output_contract.py which is outside the
# agentic-system package, so we add the project root to sys.path if needed.
_project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if _project_root not in sys.path:
    sys.path.insert(0, _project_root)

from backend.models.output_contract import (
    ResponseContract,
    Grounding,
    Evidence,
    Claim,
...
def build_output_contract(
    ...
    Returns:
        A validated ResponseContract Pydantic object.
    """
    logger.info("Building output contract from response and context")

    # Detect answer type
    answer_type_str = detect_answer_type(response, aufgaben)
    ...
    if active_beats:
        ...
        for sentence in sentences:
            ...
            if match_result:
                ...
                # Re-use existing evidence entry for the same beat
                evidence_idx = next(
                    (i for i, ev in enumerate(evidence_list) if ev.beat_id == beat.beat_id),
                    None
                )
