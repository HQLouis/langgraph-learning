...
This folder tests the "Satzbau berücksichtigen" appendix Eigenschaft
via the legacy per-feature test pattern. Coverage has been moved to
the registry-driven matrix:

- **Requirement:** `R-00-02` — "Use simple clear sentences; match the child's syntactic level"
...
1. Live-run the matrix against R-00-02 and collect the HTML report.
...
3. If the matrix fully subsumes the legacy coverage, mark tests here
   with `@pytest.mark.skip(reason="subsumed by matrix R-00-02")` as a
