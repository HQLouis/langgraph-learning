...
This folder tests the "flüssiger Übergang zwischen den Aufgaben"
Eigenschaft (#6) via the legacy per-feature test pattern. Coverage has
been moved to the registry-driven matrix:

- **Requirement:** `R-06-03` — "Add context before transitioning to the next scene"
...
1. Live-run the matrix against R-06-03 and collect the HTML report.
...
3. If the matrix fully subsumes the legacy coverage, mark tests here
   with `@pytest.mark.skip(reason="subsumed by matrix R-06-03")` as a
