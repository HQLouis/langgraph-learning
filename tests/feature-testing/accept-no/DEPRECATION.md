...
This folder tests the "Nein akzeptieren" Eigenschaft (#19) via the
legacy per-feature test pattern. Coverage has been moved to the
registry-driven matrix:

- **Requirement:** `R-19-01` — "Accept 'Nein' and continue the conversation"
...
1. Live-run the matrix against R-19-01 and collect the HTML report.
...
3. If the matrix fully subsumes the legacy coverage, mark tests here
   with `@pytest.mark.skip(reason="subsumed by matrix R-19-01")` as a
